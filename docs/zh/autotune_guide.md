...
设置 `TRITON_PRINT_AUTOTUNING=1` 后，原有调优日志保持不变，包括自动生成配置的粗测、测速结果和编译失败缓存命中日志。在此基础上，为显式传入 `configs` 的配置（包括通过 `hints` 展开的配置）补充原先缺少的耗时、失败、剪枝、中断或缓存命中反馈；新增反馈仅针对用户配置。

| 情况 | 反馈内容 |
| --- | --- |
| 测速成功 | 耗时，以及是否为最优配置 |
| 被剪枝 | 剪枝阶段或时间预算等已有原因 |
| 编译或测速失败 | 失败阶段、异常类型及简短错误信息 |
| 流程中断 | 未完成的阶段和已知中断原因；不推测具体编译错误 |
| 只剩一个配置，直接执行成功 | 说明未进行测速，不虚构耗时 |

命中缓存时，只展示现有记录，不重新编译或测速来补全诊断：

- **内存调优缓存**只按 key 保存最优配置，因此报告该配置已被选中；其他用户配置说明因缓存命中跳过。不会使用其他 key 最近一次的耗时记录。
- **磁盘调优缓存**中的用户配置耗时标注 `source=disk_cache`；没有记录的配置输出 `skipped_cache_hit`，不推测其历史失败原因。
- **编译失败缓存**命中时，输出已保存的异常类型，并说明未重试。

这些反馈不改变缓存格式、缓存 key、配置选择或异常传播。命中调优缓存后，选中的 kernel 仍按原流程执行。
